...
/* ARGV[1] 파일의 내용을 16진수와 ASCII 형태로 시스템 콘솔에 출력한다. */
void
...
/* "scratch" 디스크, hdc 또는 hd1:0에서 파일 시스템의 ARGV[1] 파일로 복사한다.
 *
 * scratch 디스크의 현재 섹터는 "PUT\0" 문자열로 시작해야 하며, 그 뒤에는
 * 파일 크기를 바이트 단위로 나타내는 32비트 little-endian 정수가 온다.
 * 이후 섹터들은 파일 내용을 담는다.
 *
 * 이 함수의 첫 호출은 scratch 디스크의 시작부터 읽는다. 이후 호출은 디스크를
 * 따라 앞으로 진행한다. 이 디스크 위치는 fsutil_get()에서 사용하는 위치와
 * 독립적이므로 모든 `put'은 모든 `get'보다 앞서야 한다. */
void
 ...
 *
 * scratch 디스크의 현재 섹터에는 "GET\0"이 기록되고, 그 뒤에는 파일 크기를
 * 바이트 단위로 나타내는 32비트 little-endian 정수가 기록된다.
 * 이후 섹터들은 파일 데이터를 받는다.
 *
 * 이 함수의 첫 호출은 scratch 디스크의 시작부터 쓴다. 이후 호출은 디스크를
 * 따라 앞으로 진행한다. 이 디스크 위치는 fsutil_put()에서 사용하는 위치와
 * 독립적이므로 모든 `put'은 모든 `get'보다 앞서야 한다. */
void
