/* lib/kernel/list.c용 테스트 프로그램.

   Pintos의 다른 곳에서 충분히 테스트되지 않는 리스트 기능을 테스트하려고
   시도한다.
      ...
      for (repeat = 0; repeat < 10; repeat++) 
        {
          ...
          /* 섞은 뒤 list_insert_ordered()로 삽입하고 정렬 상태를 검증한다. */
          shuffle (values, size);
          list_init (&list);
          for (i = 0; i < size; i++)
...
/* 값 A가 값 B보다 작으면 true, 아니면 false를 반환한다. */
static bool
...
/* LIST를 정방향으로 순회할 때 0...SIZE 값이 들어 있는지 검증한다. */
static void
...
/* LIST를 역방향으로 순회할 때 0...SIZE 값이 들어 있는지 검증한다. */
static void
