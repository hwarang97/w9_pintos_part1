...
/* page map level 4인 pml4 안에서 virtual address VADDR에 대한 page table
 * entry의 주소를 반환한다. PML4E가 VADDR용 page table을 가지고 있지 않다면
 * 동작은 CREATE에 따라 달라진다. CREATE가 true이면 새 page table을 만들고
 * 그 안의 포인터를 반환한다. 아니면 null 포인터를 반환한다. */
uint64_t *
...
/* kernel virtual address에 대한 mapping은 있지만 user virtual address에 대한
 * mapping은 없는 새 page map level 4(pml4)를 만든다.
 * 새 page directory를 반환하고, 메모리 할당이 실패하면 null 포인터를 반환한다. */
uint64_t *
pml4_create (void) {
...
/* kernel 것을 포함해 사용 가능한 각 pte entry에 FUNC를 적용한다. */
bool
...
/* pml4e를 파괴하고 그것이 참조하는 모든 page를 해제한다. */
void
pml4_destroy (uint64_t *pml4) {
	...
	/* 정의상 PML4(vaddr) >= 1이면 kernel space다. */
...
/* page directory PD를 CPU의 page directory base register에 load한다. */
void
 ...
 * 그 physical address에 대응하는 kernel virtual address를 반환하거나,
 * UADDR가 mapping되어 있지 않으면 null 포인터를 반환한다. */
void *
...
/* page map level 4 PML4에 user virtual page UPAGE에서 kernel virtual address
 * KPAGE가 식별하는 physical frame으로 가는 mapping을 추가한다.
 * UPAGE는 이미 mapping되어 있으면 안 된다. KPAGE는 보통 palloc_get_page()로
 * user pool에서 얻은 page여야 한다.
 * WRITABLE이 true이면 새 page는 read/write이고, 아니면 read-only다.
 * 성공하면 true, 메모리 할당이 실패하면 false를 반환한다. */
bool
 ...
 * 이후 이 page에 접근하면 fault가 발생한다. page table entry의 다른 bit는 보존된다.
 * UPAGE는 mapping되어 있지 않아도 된다. */
void
...
/* PML4 안의 virtual page VPAGE에 대한 PTE가 dirty이면 true를 반환한다.
 * 즉 PTE가 설치된 뒤 page가 수정되었는지를 뜻한다.
 * PML4가 VPAGE에 대한 PTE를 포함하지 않으면 false를 반환한다. */
bool
...
/* PML4 안의 virtual page VPAGE에 대한 PTE에서 dirty bit를 DIRTY로 설정한다. */
void
...
/* PML4 안의 virtual page VPAGE에 대한 PTE가 최근 접근되었으면 true를 반환한다.
...
/* PD 안의 virtual page VPAGE에 대한 PTE에서 accessed bit를 ACCESSED로 설정한다. */
void
